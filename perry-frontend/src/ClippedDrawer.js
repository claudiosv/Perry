...
export class ClippedDrawer extends React.Component {
    ...
    fetch('http://localhost:3000/devices', {
      ...
      .then((data) => {
        const devices = data.map(device => (
          <ListItem button key={device.topic} onClick={e => this.handleClick(device.topic, e)}>
            <ListItemIcon>
              <GpsFixedIcon />
            </ListItemIcon>
  ...
  render() {
    ...
    const height = this.toolbarElement && this.toolbarElement.getBoundingClientRect().height;
    return (
...
Devices
        ...
        <main className={classes.content}>
          <div className={classes.toolbar} ref={el => (this.toolbarElement = el)} />
